...
app.get('/', (req, res)=>{
    ...
    db.query('SELECT * from teacher',(err, teacher)=>{
        ...
        {
            db.query(`SELECT teacher.id as teacher_id, name, subject, class, room from 
            teacher join office on office_no = office.id
...
app.post('/create_process', (req, res)=>{
    ...
    req.on('end', ()=>{
        ...
        db.query(`insert into teacher (name, subject, created, class, 
            office_no) values (?, ?, now(), ?, ?)`, [title, description, null, 1],(err, result)=>{
            if (err) throw err;
